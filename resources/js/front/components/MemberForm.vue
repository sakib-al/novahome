...
<!--        <div class="col-md-4" v-if="parseInt(is_free)===1">
        ...
        <div class="col-md-4" v-if="member_type_id && !parseInt(member_type.is_free)">
            <div class="form-group">
                <label><b>Payable Amount(USD) *</b></label>
                <input class="form-control" name="amount" :value="member_type.amount" type="text" readonly  required>
            </div>
        </div>
        <template v-for="(item,key) in items">
            ...
            <div class="col-md-4" v-if="item.input_type==='file'">
                <div class="form-group">
                    <label><b>{{ item.label }}</b></label>
                    <input class="form-control" :name="item.input_name" type="file" :placeholder="item.input_name" >
                </div>
            </div>
    ...
    methods:{
        ...
        findItem(){
            if (this.member_type_id){
                let index = this.member_types.findIndex(item=>item.id===this.member_type_id);
                if (index>=0){
                    this.member_type=this.member_types[index];
